package main

import (
	"bytes"
	"context"
	"encoding/json"
	"errors"
	"fmt"
	"io"
	"log"
	"net/http"
	"os"
	"strings"
	"time"

	"github.com/aws/aws-sdk-go-v2/config"
	"github.com/aws/aws-sdk-go-v2/service/s3"
)

type reduceResponse struct {
	RunID      string `json:"run_id"`
	Output     string `json:"output"`
	DurationMS int64  `json:"duration_ms"`
}

func main() {
	ctx := context.Background()

	cfg, err := config.LoadDefaultConfig(ctx)
	if err != nil {
		log.Fatalf("aws config: %v", err)
	}
	s3c := s3.NewFromConfig(cfg)

	mux := http.NewServeMux()

	mux.HandleFunc("/healthz", func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		w.Write([]byte("ok"))
	})

	mux.HandleFunc("/reduce", func(w http.ResponseWriter, r *http.Request) {
		start := time.Now()

		inputs := r.URL.Query()["input"]
		if len(inputs) == 0 {
			http.Error(w, "missing query param: input (repeat 3 times)", http.StatusBadRequest)
			return
		}

		outBucket := r.URL.Query().Get("out_bucket")
		outPrefix := r.URL.Query().Get("out_prefix")
		if outPrefix == "" {
			outPrefix = "results"
		}
		outPrefix = strings.Trim(outPrefix, "/")

		reqCtx, cancel := context.WithTimeout(r.Context(), 60*time.Second)
		defer cancel()

		final := map[string]int{}

		for _, in := range inputs {
			inBucket, inKey, err := parseS3URL(in)
			if err != nil {
				http.Error(w, err.Error(), http.StatusBadRequest)
				return
			}
			if outBucket == "" {
				outBucket = inBucket
			}

			obj, err := s3c.GetObject(reqCtx, &s3.GetObjectInput{
				Bucket: &inBucket,
				Key:    &inKey,
			})
			if err != nil {
				http.Error(w, fmt.Sprintf("GetObject failed: %v", err), http.StatusBadRequest)
				return
			}

			body, err := io.ReadAll(obj.Body)
			obj.Body.Close()
			if err != nil {
				http.Error(w, fmt.Sprintf("read body failed: %v", err), http.StatusInternalServerError)
				return
			}

			var partial map[string]int
			if err := json.Unmarshal(body, &partial); err != nil {
				http.Error(w, fmt.Sprintf("json unmarshal failed: %v", err), http.StatusBadRequest)
				return
			}

			for k, v := range partial {
				final[k] += v
			}
		}

		outBytes, err := json.MarshalIndent(final, "", "  ")
		if err != nil {
			http.Error(w, fmt.Sprintf("json marshal failed: %v", err), http.StatusInternalServerError)
			return
		}

		runID := fmt.Sprintf("%d", time.Now().UTC().UnixNano())
		outKey := fmt.Sprintf("%s/%s/final.json", outPrefix, runID)

		_, err = s3c.PutObject(reqCtx, &s3.PutObjectInput{
			Bucket:      &outBucket,
			Key:         &outKey,
			Body:        bytes.NewReader(outBytes),
			ContentType: strPtr("application/json"),
		})
		if err != nil {
			http.Error(w, fmt.Sprintf("PutObject failed: %v", err), http.StatusInternalServerError)
			return
		}

		resp := reduceResponse{
			RunID:      runID,
			Output:     fmt.Sprintf("s3://%s/%s", outBucket, outKey),
			DurationMS: time.Since(start).Milliseconds(),
		}
		writeJSON(w, resp)
	})

	port := os.Getenv("PORT")
	if port == "" {
		port = "8080"
	}
	log.Printf("reducer listening on :%s", port)
	log.Fatal(http.ListenAndServe(":"+port, mux))
}

func parseS3URL(s string) (bucket, key string, err error) {
	if !strings.HasPrefix(s, "s3://") {
		return "", "", errors.New("input must start with s3://")
	}
	rest := strings.TrimPrefix(s, "s3://")
	parts := strings.SplitN(rest, "/", 2)
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return "", "", errors.New("invalid s3 url, expected s3://bucket/key")
	}
	return parts[0], parts[1], nil
}

func writeJSON(w http.ResponseWriter, v any) {
	w.Header().Set("Content-Type", "application/json")
	enc := json.NewEncoder(w)
	enc.SetIndent("", "  ")
	enc.Encode(v)
}

func strPtr(s string) *string { return &s }
